...
2    dim     当前背光亮度值(0-100)     1.dim=50 2.dim=dim+10 3.dim=dim-10
                                ...
                                1.dims=50 2.dims=dims+10 3.dims=dims-
3    dims    上电默认背光亮度值(0-100)
                                10

                                baud=9600备注: 设备支持的波特率有: 2400
             当前波特率值(本次修改，掉
4    baud                       4800 9600 19200 38400 57600
             ...
             上电默认波特率值（掉电后
5    bauds   保存，下次上电后继续有        bauds=9600
             效）

                                spax=2备注：仅对xstr指令写出来的字符有
             字符显示横向间距(上电默认
6    spax                       效，控件带的字符显示间距由控件内部的属性
             为0)
                                决定。

                                spay=2备注：仅对xstr指令写出来的字符有
             字符显示纵向间距(上电默认
7    spay                       效，控件带的字符显示间距由控件内部的属性
             为0)
                                决定。
...
9    thdra   触摸绘图功能开关           thdra=0 (关闭) thdra=1(打开)
             ...
             无串口数据自动睡眠时间(单
             位: 秒,最小3，最大65535，
10   ussp                       ussp=30(30秒无串口数据自动进入睡眠模式)
             上电默认0[0为关闭串口数据
             超时自动睡眠])
